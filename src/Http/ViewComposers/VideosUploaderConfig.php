...
class VideosUploaderConfig
{
    ...
    {
        $libraryDisk = $this->config->get('twill.video_library.disk');
        $endpointType = $this->config->get('twill.video_library.endpoint_type');
        $allowedExtensions = $this->config->get('twill.videos-library.allowed_extensions');

        // anonymous functions are used to let configuration dictate
        // the execution of the appropriate implementation
        $endpointByType = [
            'local' => function () {
                return $this->urlGenerator->route('admin.video-library.videos.store');
            },
        ...
        $signatureEndpointByType = [
            'local' => null,
            's3' => $this->urlGenerator->route('admin.video-library.sign-s3-upload'),
            'azure' => $this->urlGenerator->route('admin.video-library.sign-azure-upload'),
        ];

        $videosUploaderConfig = [
            'endpointType' => $endpointType,
            'endpoint' => $endpointByType[$endpointType](),
            'successEndpoint' => $this->urlGenerator->route('admin.video-library.videos.store'),
            ...
            'acl' => $this->config->get('twill.video_library.acl'),
            'filesizeLimit' => $this->config->get('twill.video_library.filesize_limit'),
            'allowedExtensions' => $allowedExtensions,
        ];

        $view->with(compact('videosUploaderConfig'));
    }
}
